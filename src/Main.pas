...
//	ell.width:= 2;
	...
	ball.xPosition := ScreenWidth()/2 - BitmapWidth(BitmapNamed(nameOfBitmap[0]))/2;
	ball.yPosition:= 635;
ball.isInAir:= false;
launched:= false;
...
k:= BitmapCellHeight(toDraw.bmp);
	...
	//	y+=2;
		ball.isInAir:= true;
		speed.inYDirection:= (20 * sin(DegToRad(60)));    //RANGE ADJUSMENT
		speed.inXDirection:= 0 * cos(DegToRad(60));
		speed.inZdirection:= 20 * cos(DegToRad(60));
		time:= speed.inZdirection / cos(DegToRad(60));
		end;
		end;
	//	else ball.isInAir:= false;                                             
		if(ball.isInAir) then
		begin
		ball.yPosition:= (previousBall.yPosition + (BitmapHeight(previousBall.currentBall) - BitmapHeight(ball.currentBall))); //something wrong with this
		ball.yPosition:= ball.yPosition -  speed.inYDirection * time - time*time;
		ball.xPosition:= previousBall.xPosition + (BitmapWidth(previousBall.currentBall) - (BitmapWidth(ball.currentBall)))/2;
		ball.xPosition:= ball.xPosition - speed.inXDirection*time;
		speed.inYDirection:= speed.inYDirection-2;
		ball.zPosition:= ball.zPosition + speed.inZdirection * time;
		t:= t + cos(DegToRad(60));
		i:= floor(t);
	//	if(k mod 3=0) then
	//	i+=1;
...
//if(speed.inYDirection<0) then i-=1;
	...
	(*	if(MouseClicked(RightButton)) then
		...
		if(speed.inYDirection <= (-1) * 20 * sin(DegToRad(60)) )  then //ANGLE ADJUSMENT change 70 to change hieght of projectile
		begin
	...
	ell.width:= 2;
		...
		DrawText(FloatToStr(k), ColorWhite, 50,50);
		RefreshScreen(60);
		if(i>=50) then
		begin
			i:=1;
