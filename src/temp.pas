...
BitmapSetCellDetails(BitmapNamed(fine), 250,250,10,5,50);
	...
	//	y+=2;
		ball.isInAir:= true;
		speed.inYDirection:= 100 * sin(DegToRad(45));    //RANGE ADJUSMENT
		speed.inXDirection:= 15 * cos(DegToRad(75));
		end;
...
//if(speed.inYDirection<0) then i-=1;
	...
	(*	if(MouseClicked(RightButton)) then
		...
		if(speed.inYDirection < 0.75 * (100 * ballangle * (-1)))  then //ANGLE ADJUSMENT change 70 to change hieght of projectile
		begin
